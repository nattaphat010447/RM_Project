...
const AdminMLTraining = () => {
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case 'RUNNING':
        return <span className="badge-info animate-pulse">Running</span>;
      case 'COMPLETED':
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-lumina-surface flex items-center justify-center font-jakarta font-semibold text-xl text-lumina-text-muted">Loading training status...</div>
    );
  }
  ...
  const thClass = "px-5 py-3.5 font-inter text-xs font-semibold uppercase tracking-wider text-lumina-text-muted";
  const tdCenterClass = "px-5 py-4 text-center font-inter text-sm text-lumina-text";
    ...
    <div className="min-h-screen bg-lumina-surface">
      ...
      <main className="md:ml-64 min-h-screen p-4 md:p-8 lg:p-10">
        ...
        <header className="flex justify-between items-start mb-6 gap-4">
          <div>
            <h2 className="font-jakarta font-extrabold tracking-tight text-3xl md:text-4xl text-lumina-text">Model Training</h2>
            <p className="font-jakarta text-base text-lumina-text-muted mt-1">Retrain MB-CGCN recommendation model.</p>
          </div>
          <div className="flex items-start gap-3 shrink-0">
            ...
            <button onClick={() => fetchStatus()} className="inline-flex items-center gap-2 border border-lumina-outline/60 hover:bg-white bg-white/60 text-lumina-text font-inter text-sm font-semibold px-5 py-2.5 rounded-full shadow-lumina-sm transition-colors whitespace-nowrap">
        ...
        <div className="bg-white rounded-2xl shadow-lumina-sm border border-lumina-outline/30 p-6 md:p-7 mb-6">
          <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
            <h3 className="font-jakarta font-bold text-xl text-lumina-text">Current Status</h3>
          ...
          {isTraining ? (
            <div className="bg-status-requested/10 border border-status-requested/40 rounded-xl p-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-jakarta font-bold text-xl text-status-requested">Training in Progress...</h4>
                  <p className="font-inter text-sm text-lumina-text-muted mt-1">Please wait. This may take 10-30 minutes.</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-status-available/10 border border-status-available/40 rounded-xl p-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h4 className="font-jakarta font-bold text-xl text-status-available">Latest Training Completed</h4>
                  <p className="font-inter text-sm text-lumina-text-muted mt-1">Completed: {formatDate(latestLog.completed_at)}</p>
                  {latestLog.final_recall_at_10 && (
                    <div className="mt-3 flex flex-wrap gap-x-8 gap-y-2">
                      <div>
                        <span className="font-inter text-sm text-lumina-text-muted">Recall@10:</span>
                        <span className="ml-2 font-jakarta font-extrabold text-lg text-lumina-primary">{(latestLog.final_recall_at_10 * 100).toFixed(2)}%</span>
                      </div>
                      <div>
                        <span className="font-inter text-sm text-lumina-text-muted">NDCG@10:</span>
                        <span className="ml-2 font-jakarta font-extrabold text-lg text-lumina-primary">{(latestLog.final_ndcg_at_10 * 100).toFixed(2)}%</span>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="bg-lumina-surface-alt border border-lumina-outline/50 rounded-xl p-6">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div>
                  <h4 className="font-jakarta font-bold text-xl text-lumina-text">Ready to Train</h4>
                  <p className="font-inter text-sm text-lumina-text-muted mt-1">No training in progress</p>
                </div>
                <button
                  onClick={() => handleRetrain('v1')}
                  disabled={isTraining}
        ...
        {/* Model Comparison */}
        <h3 className="font-jakarta font-extrabold text-2xl text-lumina-text mb-4">Model Comparison</h3>
              ...
              <tbody className="divide-y divide-lumina-outline/30">
                ...
                <tr className="hover:bg-lumina-surface-alt/60 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center">
                    {latestV1 ? getStatusBadge(latestV1.status) : <span className="font-inter text-xs text-lumina-text-muted">Not trained</span>}
                  </td>
                  <td className="px-5 py-4 text-center">
                    {latestV1?.final_recall_at_10 ? (
                      <span className="font-inter text-sm font-bold text-lumina-primary">{(latestV1.final_recall_at_10 * 100).toFixed(2)}%</span>
                    ) : (
                  ...
                  <td className="px-5 py-4 text-center">
                    {latestV1?.final_ndcg_at_10 ? (
                      <span className="font-inter text-sm font-bold text-lumina-primary">{(latestV1.final_ndcg_at_10 * 100).toFixed(2)}%</span>
                    ) : (
                      <span className="font-inter text-sm text-lumina-text-muted">-</span>
                    )}
                  </td>
                ...
                <tr className="hover:bg-lumina-surface-alt/60 transition-colors">
                  ...
                  <td className="px-5 py-4 text-center">
                    {latestV2 ? getStatusBadge(latestV2.status) : <span className="font-inter text-xs text-lumina-text-muted">Not trained</span>}
                  </td>
                  <td className="px-5 py-4 text-center">
                    {latestV2?.final_recall_at_10 ? (
                      <span className="font-inter text-sm font-bold text-lumina-secondary">{(latestV2.final_recall_at_10 * 100).toFixed(2)}%</span>
                    ) : (
                      <span className="font-inter text-sm text-lumina-text-muted">-</span>
                    )}
                  </td>
                  <td className="px-5 py-4 text-center">
                    {latestV2?.final_ndcg_at_10 ? (
                      <span className="font-inter text-sm font-bold text-lumina-secondary">{(latestV2.final_ndcg_at_10 * 100).toFixed(2)}%</span>
                    ) : (
                      <span className="font-inter text-sm text-lumina-text-muted">-</span>
                    )}
                  </td>
        ...
        {/* Training History */}
        <h3 className="font-jakarta font-extrabold text-2xl text-lumina-text mb-4">Training History</h3>
        <div className="bg-white rounded-2xl shadow-lumina-sm border border-lumina-outline/30 overflow-hidden mb-6">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[1040px]">
              ...
              <tbody className="divide-y divide-lumina-outline/30">
                {logs.length === 0 ? (
                  <tr>
                    <td colSpan="10" className="py-12 font-jakarta text-lumina-text-muted italic text-center">No training records yet</td>
                  </tr>
                ) : (
                  logs.map(log => (
                    <tr key={log.id} className="hover:bg-lumina-surface-alt/60 transition-colors">
                      ...
                      <td className="px-5 py-4 text-center">
                        {log.final_recall_at_10 ? (
                          <span className="font-inter text-sm font-bold text-lumina-primary">{(log.final_recall_at_10 * 100).toFixed(2)}%</span>
                        ) : (
                          <span className="font-inter text-sm text-lumina-text-muted">-</span>
                        )}
                      </td>
                      <td className="px-5 py-4 text-center">
                        {log.final_ndcg_at_10 ? (
                          <span className="font-inter text-sm font-bold text-lumina-primary">{(log.final_ndcg_at_10 * 100).toFixed(2)}%</span>
                        ) : (
                          <span className="font-inter text-sm text-lumina-text-muted">-</span>
                        )}
                      </td>
